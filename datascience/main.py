...
generator = CampaignGen()
vendor_suggest = VendorSuggest()


@app.route('/generate', methods=['POST'])
def generate_campaign():
    data = request.get_json(force=True)
    generator.set_data(data['goal'], data['name'], data['pstype'], data['budget'], data['industry'],
...
def get_vendors():
    data = request.get_json(force=True)
    vendor_suggest.set_data(data['pref'], data['typeV'], data['mcLat'], data['mcLng'])
    response = app.response_class(
